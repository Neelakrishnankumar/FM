...
import Listviewpopup from "../components/Lookup";
...
const Editbankmasters = () => {
  ...
  const selectcelldata = (data, bMode, field) => {
    ...
    if (bMode == "A") {
      ...
      if (field == "action") {
        ...
        setselectSMLookupData({
          SMlookupRecordid: data.CurrencyID,
  ...
  const currencyWiseBankInitialvalues = {
  ...
  const fncurrencySave = async (values, resetForm, del) => {
    ...
    const idata = {
      ...
      CurRecordID: selectSMLookupData.SMlookupRecordid || 0,
      Address: values.Address || '',
      SortOrder: values.SortOrder || '',
    ...
    const response = await dispatch(
      explorePostData({ accessID: "TR024", action, idata })
    );
      ...
      <Box>
        <Box sx={{ height: "100vh", overflow: "auto" }}>
          ...
          {show == "1" ? (
            <Box m="20px" sx={{ m: 2 }}>
              <Formik
                initialValues={currencyWiseBankInitialvalues}
                enableReinitialize={true}
